(function () {
    ...
    function ocultarMensagensAutomaticamente() {
        ...
        mensagens.forEach(function (mensagem) {
            window.setTimeout(function () {
                ...
                window.setTimeout(function () { mensagem.remove(); }, 400);
            }, 4000);
        });
    }
    ...
    function inicializarGraficos() {
        ...
        if (!canvasManutencoes || !canvasOcorrencias || typeof window.Chart === 'undefined') return;
        ...
        const graficoOcorrencias = new window.Chart(canvasOcorrencias, {
            type: 'bar',
            data: {
                labels: dadosOcorr.map(function (i) { return i.codigo; }),
                datasets: [{
                    label: 'Ocorrências',
                    data: dadosOcorr.map(function (i) { return Number(i.total_ocorrencias || 0); }),
                    borderWidth: 1
                }]
            },
            options: {
                ...
                onClick: function (evento, elementos) {
                    ...
                    if (codigo) mostrarDetalhesTrem(codigo, detalhes);
                }
            }
        });
    ...
    function mostrarDetalhesTrem(codigo, detalhesTrens) {
        ...
        if (!painel || !detalhesTrens || !detalhesTrens[codigo]) return;

        const trem = detalhesTrens[codigo];
        const tipos = trem.tipos.length
            ? trem.tipos.map(function (i) { return '<li>' + escaparHtml(i.tipo) + ': ' + i.total + '</li>'; }).join('')
            : '<li>Nenhuma ocorrência registrada.</li>';
        const ocorrencias = trem.ocorrencias.length
            ? trem.ocorrencias.slice(0, 4).map(function (i) { return '<li><strong>' + escaparHtml(i.tipo) + ':</strong> ' + escaparHtml(i.descricao) + '</li>'; }).join('')
            : '<li>Nenhuma ocorrência registrada.</li>';
        const manutencoes = trem.manutencoes.length
            ? trem.manutencoes.slice(0, 4).map(function (i) { return '<li><strong>' + escaparHtml(i.componente) + ':</strong> ' + escaparHtml(i.status) + ' (' + escaparHtml(i.ordem_manutencao) + ')</li>'; }).join('')
    ...
    function inicializarMapa() {
        ...
        function trechoPorId(id) {
            return dados.trechos.find(function (t) { return Number(t.id) === Number(id); });
        }

        function pontoTrecho(trecho, percentual) {
            if (!trecho) return { x: 0, y: 0 };
        ...
        function pontoMedioTrecho(trecho) { return pontoTrecho(trecho, 50); }

        function rotaPorId(id) {
            const segmentos = dados.rotas.filter(function (r) { return Number(r.id) === Number(id) && r.trecho_id; });
            return segmentos.sort(function (a, b) { return Number(a.ordem) - Number(b.ordem); });
        }

        function obterCodigoRota(id) {
            const rota = dados.rotas.find(function (i) { return Number(i.id) === Number(id); });
        ...
        function desenharTrechos() {
            ...
            dados.trechos.forEach(function (trecho) {
                const linha = criarSvg('line', {
                    x1: trecho.origem_x, y1: trecho.origem_y,
                    x2: trecho.destino_x, y2: trecho.destino_y,
                    class: 'trecho-linha status-' + normalizarClasse(trecho.status),
                    'data-tipo': 'trecho', 'data-id': trecho.id
                });
                linha.addEventListener('click', function (e) { e.stopPropagation(); selecionarElemento('trecho', trecho); });
        ...
        function desenharRotas() {
            ...
            dados.trens.forEach(function (trem) {
                ...
                if (rotaId) rotasParaDesenhar[rotaId] = true;
            });
        ...
        function desenharEstacoes() {
            ...
            dados.estacoes.forEach(function (estacao) {
                ...
                const circulo = criarSvg('circle', { cx: estacao.posicao_x, cy: estacao.posicao_y, r: 11, class: 'estacao-ponto' });
                const rotulo = criarSvg('text', { x: estacao.posicao_x, y: Number(estacao.posicao_y) + 30, class: 'estacao-rotulo', 'text-anchor': 'middle' });
                rotulo.textContent = estacao.nome;
                grupo.appendChild(circulo);
                grupo.appendChild(rotulo);
                grupo.addEventListener('click', function (e) { e.stopPropagation(); selecionarElemento('estacao', estacao); });
        ...
        function desenharSensores() {
            ...
            dados.sensores.forEach(function (sensor, indice) {
                ...
                const circulo = criarSvg('circle', {
                    cx: ponto.x + 12, cy: ponto.y - 16 - (indice % 2) * 8,
                    r: 6, class: 'sensor-ponto status-' + normalizarClasse(sensor.status)
                });
                grupo.appendChild(circulo);
                grupo.addEventListener('click', function (e) { e.stopPropagation(); selecionarElemento('sensor', sensor); });
        ...
        function desenharAmvs() {
            ...
            dados.amvs.forEach(function (amv) {
                ...
                const quadrado = criarSvg('rect', {
                    x: ponto.x - 7, y: ponto.y - 7, width: 14, height: 14, class: 'amv-ponto'
                });
                quadrado.addEventListener('click', function (e) { e.stopPropagation(); selecionarElemento('amv', amv); });
        ...
        function desenharOcorrencias() {
            ...
            dados.ocorrencias.forEach(function (ocorrencia, indice) {
                ...
                const marcador = criarSvg('circle', {
                    cx: ponto.x - 14, cy: ponto.y - 20 - (indice % 2) * 8,
                    r: 6, class: 'ocorrencia-ponto'
                });
                marcador.addEventListener('click', function (e) { e.stopPropagation(); selecionarElemento('ocorrencia', ocorrencia); });
        ...
        function desenharManutencoes() {
            ...
            dados.manutencoes.forEach(function (manutencao, indice) {
                ...
                const marcador = criarSvg('rect', {
                    x: ponto.x + 18, y: ponto.y + 4 + (indice % 2) * 8,
                    width: 10, height: 10, class: 'manutencao-ponto'
                });
                marcador.addEventListener('click', function (e) { e.stopPropagation(); selecionarElemento('manutencao', manutencao); });
            ...
            dados.alertas.forEach(function (alerta, indice) {
                ...
                const ponto = pontoMedioTrecho(trecho);
                const marcador = criarSvg('circle', {
                    cx: ponto.x, cy: ponto.y + 22 + (indice % 2) * 8,
                    r: 7, class: 'alerta-ponto nivel-' + normalizarClasse(alerta.nivel)
                });
                marcador.addEventListener('click', function (e) { e.stopPropagation(); selecionarElemento('alerta', alerta); });
                camadas.manutencoes.appendChild(marcador);
            });
        }
        ...
        function desenharTrens() {
            ...
            dados.trens.forEach(function (trem) {
                ...
                const circulo = criarSvg('circle', {
                    cx: ponto.x, cy: ponto.y, r: 10,
                    class: 'trem-ponto status-trem-' + normalizarClasse(trem.status)
                });
                const rotulo = criarSvg('text', {
                    x: ponto.x, y: ponto.y - 16, class: 'trem-rotulo', 'text-anchor': 'middle'
                });
        ...
        function montarPainel(tipo, item) {
            ...
            if (tipo === 'trecho') {
                const sensores = dados.sensores.filter(function (s) { return Number(s.trecho_id) === Number(item.id); }).length;
                const trens = dados.trens.filter(function (t) { return Number(t.trecho_id) === Number(item.id); }).map(function (t) { return t.codigo; });
                return '<div class="detail-heading"><strong>' + escaparHtml(item.codigo) + '</strong><span class="status-badge">' + escaparHtml(item.status) + '</span></div>' +
            ...
            if (tipo === 'sensor') {
                return '<div class="detail-heading"><strong>' + escaparHtml(item.codigo) + '</strong><span class="status-badge">' + escaparHtml(item.status) + '</span></div>' +
                    ...
                    '<p><strong>Limite:</strong> ' + escaparHtml(item.limite !== null && item.limite !== undefined ? item.limite : 'Não informado') + ' ' + escaparHtml(item.unidade || '') + '</p>';
            }
            if (tipo === 'amv') {
                return '<div class="detail-heading"><strong>' + escaparHtml(item.codigo) + '</strong><span class="status-badge">' + escaparHtml(item.status) + '</span></div>' +
        ...
        function simularAlteracaoRota(tremId) {
            ...
            dados.rotas.forEach(function (rota) {
                if (rota.id && rotasDisponiveis.indexOf(Number(rota.id)) === -1) {
                    rotasDisponiveis.push(Number(rota.id));
                }
            });

            const rotaAtual = Number(estado.rotasAtivas[trem.id]);
            const novaRota = rotasDisponiveis.find(function (r) { return r !== rotaAtual; });
            if (!novaRota) return;
        ...
        function animarTremParaNovaRota(trem, rotaId) {
            ...
            if (reduzirMovimento) {
                ...
                if (rotulo) { rotulo.setAttribute('x', destino.x); rotulo.setAttribute('y', destino.y - 16); }
                return;
            }
            ...
            function passo(tempo) {
                ...
                if (rotulo) { rotulo.setAttribute('x', x); rotulo.setAttribute('y', y - 16); }
        ...
        const btnMais = document.getElementById('mapaZoomMais');
        const btnMenos = document.getElementById('mapaZoomMenos');
        const btnReset = document.getElementById('mapaReset');

        if (btnMais) btnMais.addEventListener('click', function () { aplicarZoom(0.8); });
        if (btnMenos) btnMenos.addEventListener('click', function () { aplicarZoom(1.25); });
